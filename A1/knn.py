...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    # dists = (x_train[:, None, :] - x_test[None,:, :]).pow(2).sum(dim=2).sqrt()
    
    # 注意这里的维度扩充，让两个矩阵在错开的维度上广播
    # 还有另一种方法是拆开减的平方分开算，详见CS231n作业
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    # 有问题，会导致所有结果和k=1相同
    # top_k_index = dists.argsort(dim=0)[:k]
    # for i in range(top_k_index.shape[1]):
    #   unique, counts = torch.unique_consecutive(top_k_index[:, i], return_counts=True)
    #   #unique_consecutive能避免潜在的排列顺序改变
    #   predicted_label = y_train[unique[counts.argmax()]]
...
class KnnClassifier:
    ...
    def predict(self, x_test: torch.Tensor, k: int = 1):
        ...
        y_test_pred = predict_labels(dist_matrix, self.y_train, k)
...
def knn_cross_validate(
    ...
    for k in k_choices:
      ...
      for i in range(len(x_train_folds)):
          x_train_batch = torch.cat(x_train_folds[0: i] + x_train_folds[i + 1:]).cuda()
          x_test_batch = x_train_folds[i].cuda()
          y_train_batch = torch.cat(y_train_folds[0: i] + y_train_folds[i + 1:]).cuda()
          y_test_batch = y_train_folds[i].cuda()
          classifier = KnnClassifier(x_train_batch, y_train_batch)
          temp_result = classifier.check_accuracy(x_test_batch, y_test_batch, k)
          k_result.append(temp_result)
      k_to_accuracies[k] = k_result
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
...
def knn_get_best_k(k_to_accuracies: Dict[int, List]):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    k_values = list(k_to_accuracies.keys())
    ...
    # 遍历剩余的 k 值，更新最佳 k 值
    for k in k_values[1:]:
        # 如果当前 k 对应的准确率更高，则更新最佳 k 值
        if torch.tensor(k_to_accuracies[k]).sum() > torch.tensor(k_to_accuracies[best_k]).sum():
            best_k = k
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
