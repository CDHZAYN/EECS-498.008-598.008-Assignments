...
class Conv(object):
    ...
    def forward(x, w, b, conv_param):
        ...
        ####################################################################
        # Replace "pass" statement withe your code
        pad = conv_param['pad']
        stride = conv_param['stride']
        w_flattened = w.clone().reshape(w.shape[0], -1)
        out = torch.zeros((x.shape[0], w.shape[0],
                           int(1 + (x.shape[2] + 2 * pad - w.shape[2]) / stride),
                           int(1 + (x.shape[3] + 2 * pad - w.shape[3]) / stride)),
                          dtype=torch.float64, device='cuda')
        in_pad = torch.nn.functional.pad(x.clone(), (pad, pad, pad, pad), 'constant', 0)
        for i in range(in_pad.shape[0]):
            in_item = in_pad[i]
            for x_in in range(w.shape[2] - 1, in_pad.shape[2], stride):
                for y_in in range(w.shape[3] - 1, in_pad.shape[3], stride):
                    in_item_item = in_item[:, x_in - w.shape[2] + 1: x_in + 1, y_in - w.shape[3] + 1: y_in + 1]
                    in_item_item = in_item_item.reshape(-1, 1)
                    result_item = w_flattened.mm(in_item_item) + b.clone().reshape(-1, 1)
                    result_item = result_item.reshape(-1)
                    x_out = (x_in - w.shape[2] + 1) / stride
                    y_out = (y_in - w.shape[3] + 1) / stride
                    for layer in range(w.shape[0]):
                        out[i, layer, int(x_out), int(y_out)] = result_item[layer]
        #####################################################################
        #                          END OF YOUR CODE                         #
        #####################################################################
        cache = (x, w, b, conv_param)
    ...
    def backward(dout, cache):
        ...
        stride = conv_param['stride']
        pad = conv_param['pad']
        in_pad = torch.nn.functional.pad(x, (pad, pad, pad, pad))

        H_out = 1 + (H + 2 * pad - HH) // stride
        W_out = 1 + (W + 2 * pad - WW) // stride

        dx = torch.zeros_like(in_pad, dtype=torch.float64, device='cuda')
        dw = torch.zeros_like(w, dtype=torch.float64, device='cuda')
        db = torch.sum(dout, dim=(0, 2, 3))

        for i in range(N):
            for j in range(F):
                for x_out in range(0, H_out):
                    for y_out in range(0, W_out):
                        dx[i, :, x_out * stride:x_out * stride + HH, y_out * stride:y_out * stride + WW] += w[j, :] * \
                                                                                                            dout[
                                                                                                                i, j, x_out, y_out]
                        dw[j, :] += in_pad[i, :, x_out * stride:x_out * stride + HH,
                                    y_out * stride:y_out * stride + WW] * dout[i, j, x_out, y_out]
...
class MaxPool(object):
    ...
    def forward(x, pool_param):
        ...
        out = torch.zeros(N, C, H_out, W_out, dtype=torch.float64, device='cuda')
        for i in range(N):
            for j in range(C):
                for x_out in range(H_out):
                    for y_out in range(W_out):
                        out[i, j, x_out, y_out] = torch.max(x[i, j, x_out * stride: x_out * stride + pool_height,
                                                            y_out * stride: y_out * stride + pool_width])
        ####################################################################
        #                         END OF YOUR CODE                         #
        ####################################################################
        cache = (x, pool_param)
            ...
            for j in range(C):
                for k in range(H_out):
                    for l in range(W_out):
                        ...
                        dx[i, j, k * stride:k * stride + pool_height, l * stride:l * stride + pool_width][mask] = dout[
                            i, j, k, l]
        ####################################################################
        #                          END OF YOUR CODE                        #
        ####################################################################
...
class ThreeLayerConvNet(object):
    ...
    def __init__(self,
        ...
        # 解释一下上面的IMPORTANT：假设第一个卷积层的填充（padding）和步幅（stride）被选择得让输入的宽度和高度保持不变。
        # 有时候我们希望在网络的初始阶段尽可能地保持输入的尺寸，这个句子就是在说明这一点。

        C, H, W = input_dims
        self.params['W1'] = \
            torch.randn(num_filters, C, filter_size, filter_size, dtype=dtype, device=device) * weight_scale
        self.params['b1'] = torch.randn(num_filters, dtype=dtype, device=device)

        # 由于宽高视作不变，则只需考虑2X2 MAX POOLING层
        self.params['W2'] = \
            torch.randn(num_filters * (H // 2) * (W // 2), hidden_dim, dtype=dtype, device=device) * weight_scale
        self.params['b2'] = torch.zeros(hidden_dim, dtype=dtype, device=device)

        self.params['W3'] = torch.zeros(hidden_dim, num_classes, dtype=dtype, device=device) * weight_scale
...
class DeepConvNet(object):
    ...
    def __init__(self,
        ...
        if weight_scale == 'kaiming':
            for i in range(self.num_layers - 1):
                self.params[f'W{i + 1}'] = kaiming_initializer(filters[i], filters[i + 1], K=filter_size, relu=True,
            ...
            self.params[f'W{self.num_layers}'] = kaiming_initializer(
                filters[-1] * (H // (2 ** len(max_pools))) * (W // (2 ** len(max_pools))), num_classes, relu=False,
                dtype=dtype, device=device)
            self.params[f'b{self.num_layers}'] = torch.zeros(num_classes, dtype=dtype, device=device)
        else:
            for i in range(self.num_layers - 1):
                self.params[f'W{i + 1}'] = torch.randn(filters[i + 1], filters[i], filter_size, filter_size,
            ...
            self.params[f'W{self.num_layers}'] = torch.randn(
                filters[-1] * (H // (2 ** len(max_pools))) * (W // (2 ** len(max_pools))), num_classes, dtype=dtype,
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers - 1):
            ...
            if self.batchnorm:
                ...
                if i in self.max_pools:
                    x, caches[i] = Conv_BatchNorm_ReLU_Pool.forward(x, w, b, gamma, beta, conv_param, self.bn_params[i],
                                                                    pool_param)
                else:
        ...
        dh, grads[f'W{self.num_layers}'], grads[f'b{self.num_layers}'] = Linear.backward(dout,
        ...
        for i in range(self.num_layers - 1, 0, -1):
            if self.batchnorm:
                if i - 1 in self.max_pools:
                    dh, grads[f'W{i}'], grads[f'b{i}'], grads[f'gamma{i}'], grads[
                        f'beta{i}'] = Conv_BatchNorm_ReLU_Pool.backward(dh, caches[i - 1])
                else:
                    dh, grads[f'W{i}'], grads[f'b{i}'], grads[f'gamma{i}'], grads[
                        f'beta{i}'] = Conv_BatchNorm_ReLU.backward(dh, caches[i - 1])
            else:
                if i - 1 in self.max_pools:
...
class BatchNorm(object):
    ...
    def forward(x, gamma, beta, bn_param):
        ...
        if mode == 'train':
            ...
            # 注意此处使用**有偏估计**
            # 在pytorch中，貌似在train和test阶段均使用有偏估计
            # 猜想：https://peterchng.com/blog/2023/08/21/batchnorm-and-the-curious-case-of-training-vs.-inference-variance/
            # 论坛：https://datascience.stackexchange.com/questions/24239/batch-normalization-variance-calculation
            # 论坛关键：It looks like the original authors later implied that both training and test time should
            # use the unbiased variance. Also, the actual implementation in many libraries is not completely
            # consistent with what is described in the paper.
            sample_var = x.var(dim=0, unbiased=False)

            x_normalized = (x - sample_mean) / ((sample_var + eps) ** 0.5)

            # 适当偏移用于增强表达能力：https://blog.csdn.net/u010698086/article/details/78045532
            ...
            # 注意这里不可以 cache={...}，会生成set而不是dict
            cache = mode, mean, var, eps, x_normalized, gamma, beta, out
            ...
            x_normalized = (x - running_mean) / ((running_var + eps) ** 0.5)
    ...
    def backward(dout, cache):
        ...
        # 推导：https://blog.csdn.net/weixin_37721058/article/details/102337163
        N, D = dout.shape
        mode, mean, var, eps, x_normalized, gamma, beta, out = cache
        ...
        dL_div_x_n = dout * gamma

        var = var + eps
        # 将 x_i - μ 转换成 bar(x_i)的表示，可以少传一个参数
        x_sub_mean = x_n * (var ** 0.5)

        if mode == 'train':
            # 第一部分：x_n
            dx_n_div_x = 1 / (var ** 0.5)
            dL_div_x1 = dL_div_x_n * dx_n_div_x
            # 第二部分：方差
            dx_n_div_var = (-1 / 2) * x_sub_mean * (var ** -1.5)
            dL_div_var = torch.sum(dL_div_x_n * dx_n_div_var, dim=0)
            dvar_div_x = 2 * x_sub_mean / N
            dL_div_x2 = dL_div_var * dvar_div_x
            # 第三部分：均值
            # 1. x_n
            dx_n_div_mean = -1 * (var ** -0.5)
            dL_div_mean_1 = torch.sum(dL_div_x_n * dx_n_div_mean, dim=0)
            # 2. var
            dvar_div_mean = -2 * torch.sum(x_sub_mean, dim=0) / N
            dL_div_mean_2 = dL_div_var * dvar_div_mean
            # 第三部分汇总
            dmean_div_x = 1 / N
            dL_div_x3 = (dL_div_mean_1 + dL_div_mean_2) * dmean_div_x

            dx = dL_div_x1 + dL_div_x2 + dL_div_x3
        else:
    ...
    def backward_alt(dout, cache):
        ...
        if mode == 'train':
            # 其实就是对上述过程的总结
            dx = (N * dx_n - torch.sum(dx_n, dim=0) - x_n * torch.sum(dx_n * x_n, dim=0)) / (N * (var + eps) ** 0.5)
        else:
            dx = torch.sum(dout * gamma, dim=0)
