...
class Linear(object):
    ...
    def forward(x, w, b):
        ...
        out = x.reshape(x.shape[0], -1).mm(w) + b.t()
        ######################################################################
        #                        END OF YOUR CODE                            #
        ######################################################################
        cache = (x, w, b)
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3 * 32 * 32, num_classes=10,
        ...
        for i in range(2, self.num_layers):
            self.params[f'W{i}'] = torch.randn(hidden_dims[i - 2], hidden_dims[i - 1], dtype=dtype,
                                               device=device) * weight_scale
            self.params[f'b{i}'] = torch.zeros(hidden_dims[i - 1], dtype=dtype, device=device)
        self.params[f'W{self.num_layers}'] = torch.randn(hidden_dims[self.num_layers - 2], num_classes, dtype=dtype,
                                                         device=device) * weight_scale
...
def create_solver_instance(data_dict, dtype, device):
    ...
    # 还有很多其他设置，详情参见eecs598/solver.py
    solver = Solver(model, data_dict, optim_config={
        'learning_rate': 1e-1,
    }, lr_decay=0.95, batch_size=200, num_epochs=50, device=device)
    ##############################################################
    #                    END OF YOUR CODE                        #
    ##############################################################
    return solver
...
def get_five_layer_network_params():
    ...
    # model achieves 100% training accuracy within 20 epochs.      #
    ################################################################
    learning_rate = 0.3  # Experiment with this!
