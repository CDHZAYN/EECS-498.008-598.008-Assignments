...
import random
import statistics
from linear_classifier import sample_batch, softmax_loss_vectorized
...
def nn_forward_pass(params: Dict[str, torch.Tensor], X: torch.Tensor):
    ...
    prev_hidden = X.mm(W1) + b1
    prev_hidden[prev_hidden < 0] = 0
    hidden = prev_hidden
    # print(hidden.shape, W2)
...
def nn_forward_backward(
    ...
    ############################################################################
    # Replace "pass" statement with your code
    # 和linear_classifier里的softmax_loss_vectorized完全一样
    ...
    # W2
    grads_h2 = probs
    grads_h2[torch.arange(N), y] -= 1
    grads['W2'] = h1.t().mm(grads_h2) / N + 2 * reg * W2
    grads['b2'] = torch.sum(grads_h2 / N, dim=0)

    # W1

    # h1 -= torch.max(scores, dim=1, keepdim=True).values
    # exp_h1 = torch.exp(h1)
    # grads_h1 = exp_h1 / torch.sum(exp_h1, dim=1).reshape(-1, 1)
    # grads_h1[torch.arange(N), y] -= 1
    # 上面这一串等同于下面前两行代码

    # 以下四段注释来自CS231N course note
    # hidden_layer is itself a function of other parameters and the data!
    # We need to continue backpropagation through this variable. Its gradient can be computed as:
    grads_h1 = grads_h2.mm(W2.t())
    # Now we have the gradient on the outputs of the hidden layer.
    # Next, we have to back-propagate the ReLU non-linearity.
    # 注意符号！！！！h1 < 0会报错！！
    grads_h1[h1 <= 0] = 0
    grads['W1'] = X.t().mm(grads_h1) / N + 2 * reg * W1
    grads['b1'] = torch.sum(grads_h1 / N, dim=0)
...
def nn_get_search_params():
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    learning_rates = [1.4, 1.5, 1.55]
    hidden_sizes = [256, 512, 1024]
    regularization_strengths = [0.00001, 0.0001, 0.0005, 0.001]
    learning_rate_decays = [0.92, 0.93, 0.94]
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
...
def find_best_net(
    ...
    # 以下代码“借鉴”自：https://github.com/reniowood/EECS498-007

    best_lr, best_hs, best_reg, best_lr_decay = 0.0, 0.0, 0.0, 0.0
    ...
    print('hyperparameters with best model: lr = {}, hs = {}, reg = {}, lr_decay = {}'
          .format(best_lr, best_hs, best_reg, best_lr_decay))
    #############################################################################
    #                               END OF YOUR CODE                            #
    #############################################################################
