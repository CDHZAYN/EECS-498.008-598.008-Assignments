    ...
    for i in range(num_train):
        ...
        for j in range(num_classes):
            ...
            if margin > 0:
                ...
                # L_i = margin = max{0, (W^t * X[i])[j] - (W^t * X[i])[y[i]] + 1}
                # W^t is of shape (C, D)

                # 对应(W^t * X[i])[j]，由反向传播，W^t的行点乘X[i]只会影响W^t的第j行，即W的第j列。
                # 根据加法的聚合法则，在W^t的所有行的点乘序数中任取一个z，则W^t[j][z] * X[i] = (W^t * X[i])[j]_z，
                # 因此对于W^t[j][z]，其导数应为X[i]。
                dW[:, j] += X[i]

                # 对应-(W^t * X[i])[y[i]]，由反向传播，W^t的行点乘X[i]只会影响W^t的第y[i]行，即W的第y[i]列。
                dW[:, y[i]] -= X[i]
...
def svm_loss_vectorized(
    ...
    # L_i = margin = (W^t * X[i])[j] - (W^t * X[i])[y[i]] + 1

    # shape of (N, C)
    X_mul_W = X.matmul(W)
    # 注意这里不能使用X_mul_W[:, y]，因为这会在所有行上使用索引y，而非一个x索引对应一个y索引。
    loss_score = torch.max(X_mul_W - X_mul_W[torch.arange(X.shape[0]), y].unsqueeze(1) + 1, torch.zeros_like(X_mul_W))
    loss_score[torch.arange(X.shape[0]), y] = 0
    loss = torch.sum(loss_score) / X.shape[0] + reg * torch.sum((W * W))
    ...
    # L_i = max{0, R - R[torch.arange(...), y[i]] + 1}（隐含了一个广播）。
    # 其中，R = (X * W)
    # 因此，R[i, j] = max{0, (X * W)[i, j] - (X * W)[i, y[i]] + 1}

    # 忽略max遮盖掉的项后，L_i实际使用的公式可由上推知：
    # L_i = sum_{j in A(i)} (R[i, j] - R[i, y[i]]) = sum_{j in A(i)}R[i, j] - |A(i)| * R[i, y[i]] + 1
    # 其中，A为原R中大于零的项。
    # 因此：
    #               { -|A(i)|, j == y[i],
    # dL/dR[i, j] = { 1      , j in A(i),
    #               { 0      , else

    # 对应j in A(i)的情况
    loss_score[loss_score > 0] = 1
    count_positive = torch.sum(loss_score, dim=1)

    # 对应j == y[i]的情况
    loss_score[torch.arange(X.shape[0]), y] = -count_positive

    # 计算dL/dW = dR/dW * dL/dR
    # dR/dW = X
    dW = X.t().matmul(loss_score) / X.shape[0] + 2 * reg * W
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
...
def svm_get_search_params():
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    learning_rates = [0.005, 0.0075, 0.01, 0.0125, 0.025]
...
def softmax_loss_naive(
    ...
    C = W.shape[1]
    for i in range(N):
        score = X[i].reshape(1, -1).mm(W)
        # 为防止指数过大，通常会对结果向量（矩阵）同减结果的最大项。
        score -= score.max(score)
        score_exp = torch.exp(score).reshape(-1)
        loss -= torch.log((score_exp[y[i]].item() / score_exp.sum()))
        # dW的公式详见https://blog.csdn.net/SKEEa/article/details/119727982
        for j in range(C):
            dW[:, j] += X[i] * score_exp[j] / score_exp.sum()
        dW[:, y[i]] -= X[i]
...
def softmax_loss_vectorized(
    ...
    probs = score_exp / torch.sum(score_exp, dim=1).reshape(-1, 1)
...
def softmax_get_search_params():
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    learning_rates = [0.025, 0.05, 0.1, 0.25, 0.5]
    regularization_strengths = [0.0000125, 0.000025, 0.00005, 0.00025, 0.0005]
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
